...
10. **VM Auto-Configuration (`configure_libvirt_vm`)** — If `virsh` is available and connected, the script lists all VMs, presents them in a TUI menu with numeric indices (so VM names with spaces do not break the menu), and lets the user pick one. It checks the VM XML for an existing `ivshmem-plain` device to avoid duplicate injection. If absent, it writes a temporary XML block with a 64 MB shmem device, attaches it via `virsh attach-device --config`, and warns that a **full cold shutdown** (not a restart) is required for the hardware to appear. The temporary file is protected by a trap that cleans it up on `SIGINT`/`SIGTERM`.
